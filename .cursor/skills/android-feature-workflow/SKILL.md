...
```text
.cursor/skills/android-*/SKILL.md
```
...
1. 读取 `.cursor/rules/accounting-root.mdc`。
2. 读取 `.cursor/rules/android-development.mdc`，即使当前工作目录是仓库根目录也要读取。
...
6. 当 Task 工具可用时，启动 Architecture Agent 审查技术栈、模块拆分、依赖、风险和开发顺序。
...
6. 当相关且可用时，通过 Task 工具启动 Architecture Agent 审查模块边界。
...
1. 读取 `.cursor/rules/accounting-root.mdc`、`.cursor/rules/android-development.mdc`、`overall-tech-plan.md` 和模块技术方案。
...
6. 当 Task 工具可用且工作可以安全委派时，启动 Implementation Agent 实现。
7. 当 Task 工具可用时，启动 Review / QA Agent 审查。
8. 运行范围最窄的可用验证命令。
9. 更新模块开发记录。
...
当用户报告 bug，或在工作中发现 bug 时使用。

步骤：

1. 读取 `.cursor/rules/accounting-root.mdc`、`.cursor/rules/android-development.mdc`、`overall-tech-plan.md` 和相关模块技术方案。
...
计划使用 Agents：
- Workflow Agent (主 Agent)
- Architecture Agent (Task): 如需要
- Implementation Agent (Task): 如需要
- Review / QA Agent (Task): 如需要
```
