...
默认由单个 Agent 处理。不要默认使用多 Agent。仅当用户明确要求审查、产品变更规模较大、变更修改已有核心流程、多个 PRD/设计版本可能冲突，或产品变更准备交接给 Android、iOS、Backend 时，才委派给 `product-reviewer` sub-agent（`.cursor/agents/product-reviewer.md`）进行产品一致性审查，或使用 `/product-reviewer` 显式调用。
...
- 仅在必须生成或编辑位图素材时使用 `GenerateImage`，普通 PRD 编写或 `.pen` 结构工作不要使用它。
...
- 设计稿文件名应尽量与对应 PRD 使用同一 `<feature-slug>`，例如 `prd/code-accounting.pen`；如设计稿也需要保留多版本，可追加同样的日期后缀。
