...
1. 进入项目级产品工作语境。
2. 读取根 `AGENTS.md`。
3. 不读取 Android、iOS、Backend 子目录 `AGENTS.md`。
4. 使用 `.codex/skills/product-workflow/SKILL.md`。
5. 处理项目级 PRD、设计稿、需求索引、设计索引、一致性审查和端侧影响摘要。
6. 默认由单个 Product Workflow Agent 使用 `gpt-5.5` 连续处理；仅在用户明确要求审查、需求/设计规模较大、修改核心流程、存在多版本冲突风险，或准备端侧交接时，才使用 Product Review Agent。
